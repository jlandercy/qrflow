...
class GenericDGCTestCase:
    ...
    def test_encode(self):
        payload = self.helper.dgc_encode(self.data)
        self.assertEqual(payload, self.payload)

    def test_dgc_decode(self):
        data = self.helper.dgc_decode(self.payload)
...
class TestSignature(SimpleTestCase):

    def test_signature(self):
        signature = DigitalGreenCertificateHelper.sign("Hello world!".encode())
